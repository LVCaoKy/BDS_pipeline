WITH nhadat123 AS (
    SELECT 
        IF(position(Category, 'phố') > 0, 'Nhà Mặt Phố',
            IF(position(Category, 'riêng') > 0, 'Nhà Riêng', 'Không xác định')) AS Category,
        replaceAll(Facing_Dir,'_','') AS Facing_Dir,
        Title,
        News_ID,
        News_Type,
        CASE
        ...
        END AS More
        FROM
            {{ source('RAW', '123nhadat') }}
)
SELECT * FROM nhadat123